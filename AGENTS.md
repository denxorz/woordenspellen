...
- Word list is in a visually hidden `#words` textarea (no on-screen cheating). **Begin opnieuw** always builds a fresh list from `WORD_BANK` (shuffle, no duplicates per round) at the count set by the **Aantal woorden** slider (3–min(25, bank size)). On first load the slider syncs from the default textarea (4 words).
